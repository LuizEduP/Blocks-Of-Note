...
test.describe('Tasks Page — CRUD E2E', () => {
    ...
    test('deve alterar urgência e refletir no cubo', async ({ page }) => {
        // Select 'extra' urgency
        await page.locator('#urgencySelect').selectOption('extra');
        await page.waitForTimeout(200);

        // Cube should have 'extra' class
        const cubeClass = await page.locator('#cube').getAttribute('class');
        expect(cubeClass).toContain('extra');
    });
    ...
    test.describe('Task Board', () => {
        ...
        test('deve carregar o Task Board vazio', async ({ page }) => {
            ...
            await expect(page.locator('#taskboard-stats')).toContainText('0 tarefas');
        });
        ...
        test('deve exibir tarefas no Task Board', async ({ page }) => {
            ...
            await expect(page.locator('#taskboard-stats')).toContainText('1 tarefa');
        });
        ...
        test('deve filtrar tarefas por urgência no Task Board', async ({ page }) => {
            ...
            // Filter by 'high'
            await page.locator('#taskboard-filter').selectOption('high');
            await page.waitForTimeout(300);
        ...
        test('deve buscar tarefas por texto no Task Board', async ({ page }) => {
            ...
            // Search
            await page.locator('#taskboard-search').fill('Reunião');
            await page.waitForTimeout(300);
            ...
            await page.locator('#taskboard-search').fill('Inexistente');
            await page.waitForTimeout(300);
